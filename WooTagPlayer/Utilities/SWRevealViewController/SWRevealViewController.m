...
- (void) viewDidLayoutSubviews {
    if (CURRENT_DEVICE_VERSION >= 7.0  && self.view.frame.size.height == [[UIScreen mainScreen] bounds].size.height) {
...
- (void)viewWillAppear:(BOOL)animated {
    @try {
        ...
        if (_frontViewController.navigationController.interfaceOrientation != UIInterfaceOrientationPortrait) {
            [[UIApplication sharedApplication] setStatusBarOrientation:UIInterfaceOrientationPortrait];
            UIViewController *firstViewController = [[UIViewController alloc] init];
//            [_frontViewController presentModalViewController:firstViewController animated:NO];
            [_frontViewController presentViewController:firstViewController animated:NO completion:nil];
            [firstViewController dismissViewControllerAnimated:NO completion:nil];
//            [firstViewController dismissModalViewControllerAnimated:NO];
        }
    } @catch (NSException *exception) {
